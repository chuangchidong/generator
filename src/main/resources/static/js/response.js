$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'free/response/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.response.id == null ? "free/response/save" : "free/response/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.response),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "free/response/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "free/response/info/"+id, function(r){
                vm.response = r.response;
            });
		},
