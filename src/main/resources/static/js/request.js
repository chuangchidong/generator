$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '是否能为空(1.能，0 不能)', name: 'isNullable', index: 'is_nullable', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.request.id == null ? "free/request/save" : "free/request/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "free/request/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "free/request/info/"+id, function(r){
